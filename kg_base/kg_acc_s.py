...
logger.info('计算下sub acc sub loss 用focal loss mean,调整s的权重1.5')
...
# t = Conv1D(char_size, 3,dilation_rate=2 ,activation='relu' ,padding='same')(t)
# t = Conv1D(char_size, 3,dilation_rate=4 ,activation='relu' ,padding='same')(t)
...
s1_loss = focal_loss()(s1,ps1)
s1_loss = K.sum(s1_loss * mask) / K.sum(mask)
s2_loss = focal_loss()(s2,ps2)
s2_loss = K.sum(s2_loss * mask) / K.sum(mask)
...
loss = 1.5 * (s1_loss + s2_loss) + (o1_loss + o2_loss)
