            ...
            for i in idxs:
                ...
                for sp in d['spo_list']:
                    ...
                    if subjectid != -1 and objectid != -1:
                        key = (objectid,objectid+len(sp[2]))
                        if key not in items:
                            items[key] = []
                        items[key].append((subjectid,
                                           subjectid + len(sp[0]),
                                           predicate2id[sp[1]]))
                if items:
                    ...
                    s1, s2 = [0] * len(text), [0] * len(text) # 0是unk类（共49+1个类）
                    ...
                    if len(T) == self.batch_size or i == idxs[-1]:
                        ...
                        yield [T,O1,O2,K1,K2,S1,S2], None
                        T, S1, S2, K1, K2, O1, O2, = [], [], [], [], [], [], []
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 > self.best:
            self.best = f1
            train_model.save_weights('checkpoint/best_model_object.weights')
            self.wait = 0
        else:
            self.wait += 1
        ...
        logger.info('Epoch: %d,  loss: %.5f, f1: %.4f, precision: %.4f, recall: %.4f, best f1: %.4f' % (epoch, logs.get('loss'),f1, precision, recall, self.best))
